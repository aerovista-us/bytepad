...
exports.BytePadCore = void 0;
const eventemitter3_1 = require("eventemitter3");
const uuid_1 = require("uuid");
const bytepad_types_1 = require("bytepad-types");
const bytepad_utils_1 = require("bytepad-utils");
const history_1 = require("./history");
const bytepad_storage_1 = require("bytepad-storage");
/**
...
class BytePadCore {
    ...
    constructor(config) {
        ...
        this.backupManager = new bytepad_storage_1.BackupManager(typeof window !== "undefined" ? window.localStorage : {});
    }
    /**
    ...
    async init() {
        try {
            ...
            for (const board of loaded) {
                try {
                    (0, bytepad_types_1.validateBoard)(board);
            ...
            validBoards.forEach((board) => this.boards.set(board.id, board));
    ...
    async createBoard(data) {
        try {
            ...
            const validatedData = data ? (0, bytepad_types_1.validatePartialBoard)(data) : {};
            const board = {
                id: validatedData.id ?? (0, uuid_1.v4)(),
    ...
    async updateBoard(id, data) {
        try {
            ...
            if (!existing)
                return;
            // Validate and sanitize input
            const validatedData = (0, bytepad_types_1.validatePartialBoard)(data);
            const updated = { ...existing, ...validatedData, updatedAt: Date.now() };
            // Validate the complete board
            (0, bytepad_types_1.validateBoard)(updated);
    ...
    async createNote(boardId, data) {
        try {
            ...
            // Validate and sanitize input
            const validatedData = data ? (0, bytepad_types_1.validatePartialNote)(data) : {};
            // Sanitize HTML content to prevent XSS
            const sanitizedContent = validatedData.contentHTML
                ? (0, bytepad_utils_1.sanitizeHTML)(validatedData.contentHTML)
                : "";
            const note = {
                id: validatedData.id ?? (0, uuid_1.v4)(),
            ...
            (0, bytepad_types_1.validateNote)(note);
     ...
     * ```
     */
    async updateNote(boardId, noteId, data) {
        try {
            const board = this.boards.get(boardId);
            if (!board) {
                throw new Error(`Board ${boardId} not found`);
            ...
            if (noteIndex === -1)
                return;
            // Validate and sanitize input
            const validatedData = (0, bytepad_types_1.validatePartialNote)(data);
            ...
            // Validate the complete note
            (0, bytepad_types_1.validateNote)(updated);
            // Add to history (store old state)
            this.history.push({
                type: "note",
                action: "update",
    ...
    async persistBoard(board, retries = 3) {
        let lastError;
    ...
    async undoBoard(entry) {
        if (entry.action === "create") {
            ...
            await this.updateBoard(entry.boardId, entry.data);
        }
        else if (entry.action === "delete" && entry.data) {
            // Undo delete = restore board
            await this.createBoard(entry.data);
        }
    }
    async undoNote(entry) {
        if (!entry.noteId)
            return;
        if (entry.action === "create") {
            // Undo create = delete
            ...
            // Undo update = restore old state
            await this.updateNote(entry.boardId, entry.noteId, entry.data);
        }
        else if (entry.action === "delete" && entry.data) {
            // Undo delete = restore note
            const board = this.boards.get(entry.boardId);
            if (board) {
                board.notes.push(entry.data);
                board.updatedAt = Date.now();
                await this.persistBoard(board);
                this.broadcast("noteCreated", { boardId: entry.boardId, note: entry.data });
            }
        }
    }
    async redoBoard(entry) {
    ...
    async redoNote(entry) {
        if (!entry.noteId)
            return;
        if (entry.action === "create" && entry.data) {
            // Redo create = create again
            await this.createNote(entry.boardId, entry.data);
        }
        else if (entry.action === "update" && entry.data) {
     ...
     * ```
     */
    async importLegacyData(jsonString) {
        try {
            const boards = await (0, bytepad_storage_1.importLegacyData)(jsonString);
...
exports.BytePadCore = BytePadCore;
